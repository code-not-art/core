...
export class Random {
  ...
  push(
    context: string,
    options: { seed?: string; distribution?: Distribution } = {},
  ) {
    const _seed =
      options.seed === undefined ? this.next().toString() : options.seed;
    const _distribution =
      options.distribution === undefined ? Uniform : options.distribution;
